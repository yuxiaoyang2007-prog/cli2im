...
3. **Content Guard**: Relay messages skip content guard scanning in the inbound pipeline. Relay delivery strips CTI tags with `stripCtiTags` before constructing the synthetic relay message, while `sanitizeInput` remains intentionally bypassed for `isRelay` messages to avoid applying user-input normalization to bot output.

4. **`sanitizeInput`** (line 121): Skip for relay messages. Relay delivery has already removed CTI tags, and relay text should not be re-normalized as human input:
   ```typescript
