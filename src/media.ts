...
function appendNonImageAttachmentPaths(text: string, attachments: FileAttachment[]): string {
  const lines = attachments
    ...
    .map((attachment) => {
      const label = attachment.fileName ?? attachment.localPath ?? attachment.fileKey ?? 'attachment';
      const location = attachment.localPath ?? attachment.fileKey ?? attachment.url ?? 'unavailable';
