  ...
  private async runSingleTurn(args: {
    ...
    try {
      ...
      for await (const event of events) {
        ...
        if (event.type === 'turn.completed') {
          const createdFiles = await collectNewGeneratedImages(beforeImages);
...
function formatNonImageAttachment(attachment: NonNullable<UserMessage['attachments']>[number]): string {
  const label = attachment.fileName ?? attachment.localPath ?? attachment.url ?? attachment.fileKey ?? 'attachment';
  return [
    'Non-image attachment provided to Codex:',
    `name: ${label}`,
    attachment.mimeType ? `mimeType: ${attachment.mimeType}` : undefined,
...
async function collectNewGeneratedImages(before: Set<string>): Promise<string[]> {
  const files = await readdir(GENERATED_IMAGES_DIR).catch(() => []);
  ...
  for (const file of files) {
    ...
    if (info?.isFile()) created.push(path);
  }
  return created.sort();
}
