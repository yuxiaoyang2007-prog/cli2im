...
export function handlePermissionCallback(
  ...
  const sessionKey = buildCallbackSessionKey(callback, botName);

  if (parsed.decision === 'deny') {
    return agentManager.denyPermission(sessionKey, parsed.requestId);
  }

  return agentManager.approvePermission(sessionKey, parsed.requestId);
}
